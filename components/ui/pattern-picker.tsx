  ...
  const PatternButton = ({ pattern }: { pattern: Pattern }) => {
    ...
    if (pattern.type === "image") {
      ...
      return (
        <button
          key={pattern.id}
          className={cn(
            "aspect-square rounded-md border-2 overflow-hidden relative dark-shadow bg-gray-700",
    ...
    return (
      <button
        key={pattern.id}
        className={cn(
          "aspect-square rounded-md border-2 overflow-hidden relative dark-shadow",
          isSelected ? "border-primary light-shadow" : "border-black hover:border-gray-400",
        )}
        onClick={handleClick}
      >
